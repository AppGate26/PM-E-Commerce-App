...
public class CustomerController {
    ...
    @PutMapping(value = "/admin/request-customer-edit/{customerId}")
    public BaseResponse requestCustomerEdit(@PathVariable Long customerId, @Valid @RequestBody CustomerDto customerDto) {
        return customerService.requestCustomerEdit(customerId, customerDto);
    }

    // Report Endpoints
    @GetMapping(value = "/admin/reports/customer-credit")
    public BaseResponse getCustomerCreditReport(
            @RequestParam(required = false) String startDate,
            @RequestParam(required = false) String endDate) {
        return customerService.getCustomerCreditReport(startDate, endDate);
    }

    @GetMapping(value = "/admin/reports/one-off")
    public BaseResponse getOneOffReport(
            @RequestParam(required = false) String startDate,
            @RequestParam(required = false) String endDate) {
        return customerService.getOneOffReport(startDate, endDate);
    }

    @GetMapping(value = "/admin/reports/loan-recovery")
    public BaseResponse getLoanRecoveryReport(
            @RequestParam(required = false) String startDate,
            @RequestParam(required = false) String endDate) {
        return customerService.getLoanRecoveryReport(startDate, endDate);
    }
