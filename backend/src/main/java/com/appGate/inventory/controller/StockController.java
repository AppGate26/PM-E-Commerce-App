...
public class StockController {
    ...
    public BaseResponse createStock(@RequestBody StockDto dto) {
        return stockService.createStock(dto);
    }
    ...
    // ==================== CENTRAL / BRANCH STOCK ENDPOINTS ====================

    @Operation(summary = "Get central (HQ) stock", description = "Retrieve all stock held centrally — not yet allocated to any branch")
    @GetMapping("/api/admin/stocks/central")
    public BaseResponse getCentralStock() {
        return stockService.getCentralStock();
    }

    @Operation(summary = "Get stock for a branch", description = "Retrieve all stock entries allocated to a specific branch")
    @GetMapping("/api/admin/stocks/branch/{branchId}")
    public BaseResponse getStockByBranch(@PathVariable Long branchId) {
        return stockService.getStockByBranch(branchId);
    }

    @Operation(summary = "Allocate stock to branch",
            description = "Transfer a quantity of a product from central (HQ) stock to a specific branch")
    @PostMapping("/api/admin/stocks/allocate")
    public BaseResponse allocateStockToBranch(@Valid @RequestBody StockAllocationDto dto) {
        return stockService.allocateStockToBranch(dto);
    }
}
