...
public class SupplierService {
    ...
    public BaseResponse createSupplier(SupplierDto supplierDto, HttpServletRequest request) {
        try {
            ...
            if (newSupplier.getSupplierId() == null || newSupplier.getSupplierId().isEmpty()) {
                String year = String.valueOf(Year.now().getValue()).substring(2);
                String formattedId = String.format("%06d", newSupplier.getId());
                newSupplier.setSupplierId("SUP/" + year + "/" + formattedId);
    ...
    public BaseResponse getAllSuppliers() {
        // Shared: company-wide suppliers (null branch) plus this branch's own.
        Long branchId = branchScopeService.getScopedBranchId();
        List<Supplier> suppliers =
                supplierRepository.findAll(BranchSpecs.<Supplier>visibleTo(branchId));
    ...
    /**
     * The single lookup behind get / update / delete, so guarding it here stops a
     * branch user reaching another branch's private supplier by id. Company-wide
     * suppliers (null branch) stay reachable by everyone.
     */
    private Supplier getOneSupplier(Long id) {
        ...
        branchScopeService.assertCanAccessShared(supplier.getBranchId());
        return supplier;
    }
