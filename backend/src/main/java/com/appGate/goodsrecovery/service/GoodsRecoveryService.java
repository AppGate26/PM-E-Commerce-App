...
public class GoodsRecoveryService {
    ...
    /** Load a recovery record, refusing one that belongs to another branch. */
    private GoodsRecovery recoveryInScope(GoodsRecovery recovery) {
        branchScopeService.assertCanAccess(recovery.getBranchId());
        return recovery;
    }
    ...
    public BaseResponse getPendingRecoveries(String status, int page, int size) {
        ...
        if (status != null && !status.isEmpty()) {
            ...
            recoveries = branchId == null
                    ? goodsRecoveryRepository.findByStatus(recoveryStatus, pageable)
                    : goodsRecoveryRepository.findByStatusAndBranchId(recoveryStatus, branchId, pageable);
        } else {
            recoveries = branchId == null
                    ? goodsRecoveryRepository.findAll(pageable)
                    : goodsRecoveryRepository.findByBranchId(branchId, pageable);
        }
    ...
    public BaseResponse getCustomerRecoveryDetails(Long customerId) {
        GoodsRecovery recovery = recoveryInScope(goodsRecoveryRepository.findByCustomerId(customerId)
    ...
    public BaseResponse markAsRecovered(MarkAsRecoveredDto dto, HttpServletRequest request) {
        GoodsRecovery recovery = recoveryInScope(goodsRecoveryRepository.findByCustomerId(dto.getCustomerId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "No recovery record found for this customer")));
    ...
    public BaseResponse getRecoveryReports(int page, int size) {
        ...
        Long branchId = branchScopeService.getScopedBranchId();
        Page<GoodsRecovery> recoveries = branchId == null
                ? goodsRecoveryRepository.findByStatus(GoodsRecoveryStatus.RECOVERED, pageable)
                : goodsRecoveryRepository.findByStatusAndBranchId(GoodsRecoveryStatus.RECOVERED, branchId, pageable);
    ...
    public BaseResponse getRecoveryReportDetail(Long recoveryId) {
        GoodsRecovery recovery = recoveryInScope(goodsRecoveryRepository.findById(recoveryId)
