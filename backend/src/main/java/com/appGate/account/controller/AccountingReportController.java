...
public class AccountingReportController {
    ...
    @Operation(summary = "Get branch account (journal) report",
            description = "Journal entries posted for a specific branch, optionally within a date range")
    @GetMapping("/branch/{branchId}/journal")
    public BaseResponse getBranchJournalReport(
            @PathVariable Long branchId,
            @RequestParam(required = false) LocalDate startDate,
            @RequestParam(required = false) LocalDate endDate) {
        return reportService.getBranchJournalReport(branchId, startDate, endDate);
    }
}
