...
public class RiderService {
    ...
    private Rider saveRider(RiderDto riderInfoDto, String baseUrl){
        ...
        // A RIDER with no branch is denied by the fail-closed rule, so the rider's
        // login has to be posted to the same branch that hired them.
        if (branchId != null) {
        ...
        riderInfo.setModeOfTransport(riderInfoDto.getModeOfTransport());
    ...
    public  BaseResponse getAllRidersInfo(){
        ...
        List<Rider> riders = branchId == null
                ? riderInfoRepository.findAll()
                : riderInfoRepository.findByBranchId(branchId);
        return new BaseResponse(HttpStatus.OK.value(), "successful", riders);
    }
    ...
    /**
     * The single seam every rider read/write goes through, so the branch guard
     * only has to be stated once.
     */
    private Rider getRider(Long riderId){
        Rider rider = riderInfoRepository.findById(riderId)
                .orElseThrow( ()  -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Rider not found!"));
        branchScopeService.assertCanAccess(rider.getBranchId());
        return rider;
    }
    ...
    public BaseResponse getAllSuspendedRider(boolean suspended){
        Long branchId = branchScopeService.getScopedBranchId();
        List<Rider> riders = branchId == null
                ? riderInfoRepository.findBySuspended(suspended)
                : riderInfoRepository.findBySuspendedAndBranchId(suspended, branchId);
        return  new BaseResponse(HttpStatus.OK.value(), "successful", riders);
    }
