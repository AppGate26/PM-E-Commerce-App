...
public interface DepositTransactionRepository extends JpaRepository<DepositTransaction, Long> {
    ...
    // --- Branch-scoped ---
    Page<DepositTransaction> findByBranchIdAndTransactionDateBetween(
            Long branchId, LocalDate startDate, LocalDate endDate, Pageable pageable);

    Page<DepositTransaction> findByBranchIdAndTransactionTypeAndTransactionDateBetween(
            Long branchId, TransactionType type, LocalDate startDate, LocalDate endDate, Pageable pageable);

    Page<DepositTransaction> findByBranchIdAndBankNameAndTransactionDateBetween(
            Long branchId, BankName bank, LocalDate startDate, LocalDate endDate, Pageable pageable);

    List<DepositTransaction> findByBranchIdAndCashierAndTransactionDateBetween(
            Long branchId, String cashier, LocalDate startDate, LocalDate endDate);

    @Query("SELECT SUM(d.amount) FROM DepositTransaction d WHERE d.cashier = :cashier " +
           "AND d.branchId = :branchId AND d.transactionDate BETWEEN :startDate AND :endDate")
    BigDecimal sumAmountByCashierAndBranchAndDateRange(
            @Param("cashier") String cashier,
