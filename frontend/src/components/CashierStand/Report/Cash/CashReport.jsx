...
const CashReport = ({ toggleCrModal }) => {
  ...
  const fetchCashDeposits = async () => {
    ...
    try {
      ...
      const reportsList = await cashierApi.getCashDepositsReport({
        startDate,
        endDate,
      });
      ...
      setError(err.message || "Failed to fetch cash deposits report");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Cash Deposit Report");
    XLSX.writeFile(
      workbook,
      `cash-deposit-report-${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  };
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <div class="total">Total Cash Deposits: ${formatCurrency(total)}</div>
        </body>
      </html>
    `);
