...
const DepositReport = ({ toggleDrModal }) => {
  ...
  const fetchAllDeposits = async () => {
    ...
    try {
      ...
      const reportsList = await cashierApi.getAllDepositsReport({
        startDate,
        endDate,
      });
      ...
      setError(err.message || "Failed to fetch all deposits report");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "All Deposit Report");
    XLSX.writeFile(
      workbook,
      `all-deposit-report-${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  };
  ...
  const handleExportPdf = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>All Deposit Report</title>
          <style>
        ...
        <body>
          ...
          <div class="total">Total Deposits: ${formatCurrency(total)}</div>
        </body>
      </html>
    `);
