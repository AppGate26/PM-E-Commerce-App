...
const List = ({ toggleOrdListModal }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      console.log("═══════════════════════════════════════════════════════════");
      console.log("List: FETCHING ORDER LIST NOTIFICATIONS");
      console.log("═══════════════════════════════════════════════════════════");

      const response = await apiRequest("/sales/notifications/orderlist", "GET");
      ...
      console.log(`Loaded ${notificationsList.length} order list notifications`);
      setNotifications(notificationsList);
      
    } catch (err) {
      console.error("Error fetching order list notifications:", err);
      setError(err.message || "Failed to load order list notifications.");
      setNotifications([]);
    } finally {
      setLoading(false);
    }
    ...
    <>
      <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <h1 style={styles.title}>ORDER LIST NOTIFICATIONS</h1>
                ...
                {notifications.length > 0 ? (
                  notifications.map((notification, index) => (
                    <tr key={notification.id || index}>
                      ...
                      <td style={styles.td}>
                        {notification.message || notification.notification || notification.description || "Order ready for processing"}
                      </td>
                      <td style={styles.td}>
                  ...
                  <tr>
                    <td colSpan="5" style={{ ...styles.td, textAlign: "center" }}>
                      <div style={styles.emptyState}>
                        No order list notifications found
                      </div>
                    </td>
                  </tr>
                )}
        ...
        <div style={modalStyles.overlay} onClick={closeDetailModal}>
          <div style={modalStyles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={modalStyles.header}>
              <h3 style={modalStyles.title}>Order Details</h3>
              <button
            ...
            <div style={modalStyles.content}>
              {loadingDetails ? (
                ...
                <>
                  ...
                  <div style={modalStyles.section}>
                    <h4 style={modalStyles.sectionTitle}>Notification Message</h4>
                    <div style={modalStyles.messageBox}>
                      <p style={modalStyles.messageText}>
                        {selectedNotification?.message || 
                         selectedNotification?.notification || 
                         selectedNotification?.description || 
                         "Order has been processed and is ready for pickup/delivery."}
                      </p>
                    </div>
                  </div>
