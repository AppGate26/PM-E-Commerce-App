...
const Cancelled = ({ toggleNotCancelModal }) => {
  const { user } = useAuth();
  const closeModal = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      console.log("═══════════════════════════════════════════════════════════");
      console.log("Cancelled: FETCHING CANCELLED NOTIFICATIONS");
      console.log("═══════════════════════════════════════════════════════════");

      const response = await apiRequest("/sales/notifications/cancelled", "GET");
      ...
      console.log(`Loaded ${notificationsList.length} cancelled notifications`);
      setNotifications(notificationsList);
      
    } catch (err) {
      console.error("Error fetching cancelled notifications:", err);
      setError(err.message || "Failed to load cancelled notifications.");
      setNotifications([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleViewDetails = async (notification) => {
    ...
    try {
      ...
      // Still show notification details even if order fetch fails
    } finally {
  ...
  const getOrderId = () => selectedNotification?.orderId || selectedNotification?.id || orderDetails?.id;
  ...
  const handleForwardToAdmin = async () => {
    ...
    try {
      ...
      await apiRequest(`/sales/orders/${orderId}/cancellation/forward`, "PUT", {
        comment: `Forwarded to admin by sales user ${approvedBy}`,
      });
      closeDetailModal();
      fetchNotifications();
    } catch (err) {
      setActionError(err?.message || "Unable to forward this cancellation request. Please try again.");
    } finally {
  ...
  const handleRejectRequest = async () => {
    ...
    if (!reason || reason.trim() === "") {
      setActionError("A reason is required to reject this cancellation request");
      return;
    }
    setActionLoading(true);
    setActionError("");
    try {
      ...
      setActionError(err?.message || "Unable to reject this cancellation request. Please try again.");
    } finally {
      setActionLoading(false);
    }
  };
    ...
    <>
      <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <h1 style={styles.title}>CANCELLED NOTIFICATIONS</h1>
                ...
                {notifications.length > 0 ? (
                  notifications.map((notification, index) => (
                    <tr key={notification.id || index}>
                      ...
                      <td style={styles.td}>
                        {notification.message || notification.notification || notification.reason || "Order cancelled"}
                      </td>
                      <td style={styles.td}>
                        {formatDate(notification.cancelledAt || notification.createdAt)}
                      </td>
                      <td style={styles.td}>
                        <button
        ...
        <div style={modalStyles.overlay} onClick={closeDetailModal}>
          <div style={modalStyles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={modalStyles.header}>
              <h3 style={modalStyles.title}>Cancelled Order Details</h3>
              <button
                onClick={closeDetailModal}
                style={modalStyles.closeModalBtn}
                onMouseEnter={(e) => e.target.style.backgroundColor = "rgba(255,255,255,0.16)"}
            ...
            <div style={modalStyles.content}>
              {loadingDetails ? (
                ...
                <>
                    ...
                    <div style={modalStyles.infoGrid}>
                      ...
                      <div style={modalStyles.infoItem}>
                        ...
                        <span style={modalStyles.amountValue}>
                          {formatCurrency(orderDetails?.totalAmount || orderDetails?.amount)}
                        </span>
                      </div>
                      <div style={modalStyles.infoItem}>
                        <span style={modalStyles.infoLabel}>Amount Paid</span>
                        <span style={modalStyles.paidValue}>
                          {formatCurrency(orderDetails?.amountPaid || 0)}
                        </span>
                      </div>
                      <div style={modalStyles.infoItem}>
                        <span style={modalStyles.infoLabel}>Cancelled Date</span>
                        <span style={modalStyles.infoValue}>
                          {formatDate(selectedNotification?.cancelledAt || selectedNotification?.createdAt)}
                        </span>
                      </div>
                      <div style={modalStyles.infoItem}>
                        <span style={modalStyles.infoLabel}>Status</span>
                        <span>
                          <span style={modalStyles.statusBadge}>
                  ...
                  {/* Cancellation Reason */}
                  <div style={modalStyles.section}>
                    <h4 style={modalStyles.sectionTitle}>Cancellation Reason</h4>
                  ...
                  {actionError && (
                    <div style={{ ...styles.alert, marginTop: "0" }}>{actionError}</div>
                  )}

                  {/* Sales review actions - only while this order is actually awaiting review */}
                  {orderDetails?.status === "CANCELLATION_REQUESTED" ? (
                    <div style={{ ...modalStyles.footerAction, gap: "10px" }}>
                      <button
                        onClick={handleRejectRequest}
                        disabled={actionLoading}
                        style={{
                          ...modalStyles.closeActionBtn,
                          backgroundColor: "#f44336",
                          opacity: actionLoading ? 0.6 : 1,
                        }}
                      >
                        {actionLoading ? "PLEASE WAIT..." : "REJECT REQUEST"}
                      </button>
                      <button
                      ...
                      >
                        {actionLoading ? "PLEASE WAIT..." : "FORWARD TO ADMIN"}
                      </button>
                    </div>
                  ) : (
