...
class InstallmentBreakDownScreen extends StatefulWidget {
  const InstallmentBreakDownScreen({super.key});

  @override
  State<InstallmentBreakDownScreen> createState() =>
      _InstallmentBreakDownScreenState();
}

class _InstallmentBreakDownScreenState
    extends State<InstallmentBreakDownScreen> {
  // Holds EVERYTHING passed from the pickup screen — plan + fulfillment
  // details — so nothing gets dropped on the way to the payment step.
  ...
  void didChangeDependencies() {
    ...
    if (!_extraRead) {
      ...
      _extraRead = true;
      print('💳 [INSTALLMENT BREAKDOWN] ===== didChangeDependencies =====');
      print(
          '💳 [INSTALLMENT BREAKDOWN] Plan received: planId=${_plan?.planId}');
      print('💳 [INSTALLMENT BREAKDOWN] Full extra data: $_extraData');

      if (_plan != null) {
        final plan = _plan!;
        print(
            '🟩 [INSTALLMENT BREAKDOWN] productName  = "${plan.productName}"');
        print(
            '🟩 [INSTALLMENT BREAKDOWN] productPrice (raw from model) = ${plan.productPrice}');
        print('🟩 [INSTALLMENT BREAKDOWN] insurance    = ${plan.insurance}');
        print('🟩 [INSTALLMENT BREAKDOWN] totalAmount  = ${plan.totalAmount}');
        print('🟩 [INSTALLMENT BREAKDOWN] frequency    = ${plan.frequency}');
        print(
            '🟩 [INSTALLMENT BREAKDOWN] duration     = ${plan.durationInMonths}');
        print(
            '🟩 [INSTALLMENT BREAKDOWN] resolved displayProductPrice = ${_resolvedProductPrice(plan)}');
      }
    }
  }
  ...
  /// ✅ Safety-net fallback: some hot-reload / backend edge cases can leave
  /// plan.productPrice at 0 even though the plan itself is valid. Since
  /// totalAmount (grand total) = productPrice + insurance, we can always
  /// back-derive the correct product price from those two known-good
  /// values whenever productPrice looks wrong. This never overrides a
  /// valid non-zero productPrice — it only kicks in as a last resort.
  double _resolvedProductPrice(InstallmentPlan plan) {
    ...
    print('🟡 [INSTALLMENT BREAKDOWN] productPrice was 0 — derived fallback '
        '= totalAmount(${plan.totalAmount}) - insurance(${plan.insurance}) = $derived');
  ...
  List<InstallmentSchedule> _buildDisplaySchedule(InstallmentPlan plan) {
    return InstallmentDisplayUtils.buildDisplaySchedule(plan);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (plan == null) {
      print('🔴 [INSTALLMENT BREAKDOWN] build() -> showing MISSING PLAN state');
    ...
    final displaySchedule = _buildDisplaySchedule(plan);

    print(
        '🔵 [INSTALLMENT BREAKDOWN] build() -> productName="${plan.productName}", '
        'displayProductPrice=$displayProductPrice, totalAmount=${plan.totalAmount}');
    print('🔵 [INSTALLMENT BREAKDOWN] build() -> frequency=${plan.frequency}, '
                      ...
                      children: [
                        Checkbox(
                          ...
                          onChanged: (value) {
                            ...
                            print(
                                '🔵 [INSTALLMENT BREAKDOWN] Terms accepted: $_termsAccepted');
                          },
  ...
  void _proceed(InstallmentPlan plan) {
    print('🟢 [INSTALLMENT BREAKDOWN] "Proceed to pay" TAPPED');
    print('🟢 [INSTALLMENT BREAKDOWN] Forwarding full data: $_extraData');
    context.push(
      AppRoutes.paymentFreqBreakdownPickup,
      extra: {
        ..._extraData, // carries plan + all pickup fulfillment details forward
        'plan': plan,
        'isDelivery': false,
        'termsAccepted': true,
      },
  ...
  Widget _buildFulfillmentCard() {
    final method = _extraData['deliveryMethod']?.toString() ?? 'PICKUP';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Method: ${method == 'PICKUP' ? 'Pick up from store' : method}',
  ...
  /// ✅ FIX — overflow: label and value are now Flexible/wrapped instead of
